...
In this section you will launch the workspace using a link on the Git repository. This is a particularly helpful way to get into a workspace since all the user needs is a link to the git repository. The information on how to launch the workspace is then included in the repository readme file. 
...
The relevant section of the README.md is shown below :
...
The format of the above line to add to a readme file is :

[<image specification](<dev-spaces.url>/dashboard/#/load-factory?url=<git-repository-url>)

Note that the git repository needs to have a devfile.yaml file in the root of the repository.
...
Click on the Github link below to open the Git hub repository. 
...
Scroll down the window past the file browser until you get to the README file which should match the image shown above. 

Float the mouse over the DevSpaces icon to view the URL to which it links, and then press the DevSpaces icon to launch it. You may be presented with a login dialog for the OpenShift cluster. If you are, press the blue 'Log in with OpenShift' button. Login with the provided credentials for your cluster.
